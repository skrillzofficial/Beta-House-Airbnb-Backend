const createWelcomeTemplate = (fullName, clientUrl) => {
  return `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Welcome to BetaHouse</title>
  </head>
  <body
    style="
      font-family: Arial, sans-serif;
      line-height: 1.6;
      color: #333;
      max-width: 600px;
      margin: 0 auto;
      padding: 20px;
    "
  >
    <main style="border-radius: 10px">
      <div
        style="
          background-color: #036931ff;
          padding: 30px;
          text-align: start;
          border-radius: 10px 10px 0 0;
        "
      >
        <h1 style="color: white; margin: 0; font-size: 28px">BetaHouse</h1>
      </div>
      <div
        style="
          background-color: #fafafb;
          padding: 30px;
          border-radius: 0 0 10px 10px;
        "
      >
        <p>Hi, ${fullName}</p>
        <p style="font-size: 18px; color: #000">
          <strong>Email Confirmation, Before </strong
          ><span style="color: #036931ff">We get started</span>
        </p>
        <p>
          Thank you for registering on BetaHouse, to finish your registration
          please confirm your email by clicking on the button below:
        </p>

        <div style="text-align: start; margin: 30px 0">
          <a
            href="${clientUrl}"
            target="_blank"
            style="
              background-color: #036931ff;
              color: white;
              padding: 14px 38px;
              text-decoration: none;
              border-radius: 2px;
              font-weight: bold;
              font-size: 16px;
              transition: background-color 0.3s;
              cursor: pointer;
            "
            >CONFIRM YOUR EMAIL</a
          >
        </div>
        <p>Didn't sign up for BetaHouse? Let us know.</p>
        <hr style="margin: 30px 0px" />
        <p>
          If you’re having trouble with the button above, copy and paste the URL
          below into your web browser.
        </p>
        <a herf="${clientUrl}" target="_blank" style="color: #036931ff"
          >${clientUrl}</a
        >

        <p style="font-size: 13px">Email Sent by BetaHouse</p>
      </div>
    </main>
  </body>
</html>
`;
};

const createResetTemplate = (fullName, clientUrl) => {
  return `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>BetaHouse Reset Password</title>
  </head>
  <body
    style="
      font-family: Arial, sans-serif;
      line-height: 1.6;
      color: #333;
      max-width: 600px;
      margin: 0 auto;
      padding: 20px;
    "
  >
    <main style="border-radius: 10px">
      <div
        style="
          background-color: #036931ff;
          padding: 30px;
          text-align: start;
          border-radius: 10px 10px 0 0;
        "
      >
        <h1 style="color: white; margin: 0; font-size: 28px">BetaHouse</h1>
      </div>
      <div
        style="
          background-color: #fafafb;
          padding: 30px;
          border-radius: 0 0 10px 10px;
        "
      >
        <p>Hi, ${fullName}</p>
        <p style="font-size: 22px; color: #000">
          Password Reset Confirmation: Please verify your email to proceed.
        </p>
        <p>Password Reset Confirmation: Please verify your email to proceed.</p>

        <div style="text-align: start; margin: 30px 0">
          <a
            href="${clientUrl}"
            target="_blank"
            style="
              background-color: #8a63f7;
              color: white;
              padding: 14px 38px;
              text-decoration: none;
              border-radius: 2px;
              font-weight: bold;
              font-size: 16px;
              transition: background-color 0.3s;
              cursor: pointer;
            "
            >RESET PASSWORD</a
          >
        </div>

        <hr style="margin: 30px 0px" />
        <p>
          If you’re having trouble with the button above, copy and paste the URL
          below into your web browser.
        </p>
        <a herf="${clientUrl}" target="_blank" style="color: #036931ff"
          >${clientUrl}</a
        >

        <p style="font-size: 13px">Email Sent by BetaHouse</p>
      </div>
    </main>
  </body>
</html>
`;
};

module.exports = { createResetTemplate, createWelcomeTemplate };